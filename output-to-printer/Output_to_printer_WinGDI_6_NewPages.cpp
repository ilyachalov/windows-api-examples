...
// Output_to_printer_WinGDI_6_NewPages.cpp
...
// (добавляем возможность вывода текста на несколько страниц, это может произойти
// либо по достижении текстом конца страницы, либо вставкой символа '\f')
...
int main()
{
	...
	wstring s =
		L"Земская больница. За отсутствием доктора, уехавшего жениться, больных принимает"
		L" фельдшер Курятин, толстый человек лет сорока, в поношенной чечунчовой жакетке "
		L"и в истрепанных триковых брюках. На лице выражение чувства долга и приятности. "
		L"Между указательным и средним пальцами левой руки — сигара, распространяющая зловоние.\n\n"
		L"В приемную входит дьячок Вонмигласов, высокий коренастый старик в коричневой рясе "
		L"и с широким кожаным поясом. Правый глаз с бельмом и полузакрыт, на носу бородавка,"
		L" похожая издали на большую муху. Секунду дьячок ищет глазами икону и, не найдя "
		L"таковой, крестится на бутыль с карболовым раствором, потом вынимает из красного "
		L"платочка просфору и с поклоном кладет ее перед фельдшером.\n\n"
		L"(Отрывок из рассказа А.П.Чехова «Хирургия», 1884 год.)\n\n";
	wstring s1;
	// повторим 7 раз определенный выше кусок текста, чтобы достигнуть конца первой страницы,
	// переход на вторую страницу должен быть выполнен автоматически
	for (int i = 1; i <= 7; i++)
		s1 = s1 + s;
	// проверим и работу перехода на новую страницу при вводе специального символа:
	// на третьей странице определенный выше кусок текста будет размещен в восьмой раз
	s1 = s1 + L'\f' + s;

	// вызов функции для вывода строки на принтер по умолчанию
	if (!toPrinter(s1.c_str())) // если при вызове случилась ошибка
