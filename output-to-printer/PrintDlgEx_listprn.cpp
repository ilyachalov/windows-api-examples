...
HRESULT GetPrinterFromDlgEx(HDC* printerDC, LPWSTR* printerName);
...
int main()
{
	...
	// и названия принтера
	if (GetPrinterFromDlgEx(&printerDC, &printerName) != S_OK)
	{
...
HRESULT GetPrinterFromDlgEx(HDC* printerDC, LPWSTR* printerName)
{
	...
	// функция PrintDlgExW возвратила значение S_OK, но при этом в структуре printDlgInfo
	// в поле dwResultAction может быть возвращено одно из трех значений, из которых
	// успешным мы считаем только PD_RESULT_PRINT (нажатие на кнопку "Печать" в диалоге)
	if (printDlgInfo.dwResultAction != PD_RESULT_PRINT)
		return S_FALSE;
